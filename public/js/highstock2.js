...
function kline(m, c) {
	...
	chart.main_chart = new Highcharts.StockChart({
		chart: { renderTo: 'graphbox', width: 720},
		xAxis: { type: 'datetime' },
		...
		rangeSelector: {buttons: [{type: 'minute', count: 60, text: '1h'},{type: 'minute', count: 120, text: '2h'},{type: 'minute', count: 360, text: '6h'},{type: 'minute', count: 720, text: '12h'},{type: 'day', count: 1, text: '1d'},{type: 'week', count: 1, text: '1w'},{type: 'all', text: '鎵€鏈�'}],selected:6, inputEnabled:false},
		exporting: {enabled: false, buttons: {exportButton: {enabled: false}, printButton: {enabled: true}}},
		yAxis: [
			{ labels: { style: { color: '#e55600' } }, title: { text: '浠锋牸 [RMB]', style: { color: '#e55600' }}, height: 160, lineWidth: 0, gridLineDashStyle: 'Dash', showLastLabel: false},
			{ labels: { style: { color: '#4572A7' } }, title: { text: '鎴愪氦閲� [' + c + ']', style: { color: '#4572A7' } }, offset: 0, top: 260, height: 40, lineWidth: 0, gridLineDashStyle: 'Dash', showLastLabel: true}
		],
		series: [
			{ animation: false, name: '鎴愪氦閲� [' + c + ']', type: 'column', color: '#4572A7', dataGrouping: { units: groupingUnits, enabled: false }, yAxis: 1, data: vols },
			{ animation: false, name: '浠锋牸 [RMB]', type: 'candlestick', dataGrouping: { units: groupingUnits, enabled: false }, data: rates }
		]
	});
}

$(document).ready(function(){
	var c = $("#chart-control").attr('data').toUpperCase();
	$("#chart-control > button").click(function () {
		$(this).addClass("btn-success").siblings().removeClass("btn-success");
		kline($(this).attr('data-time'), c);
	});
	if(typeof kline_m == 'undefined') kline_m = '8h';
	kline(kline_m, c);
});